 ...
 * @subpackage Document
 */
class Xlsx extends \Maho\Search\Lucene\Document\OpenXml
{
    /**
    ...
    private function __construct($fileName, $storeContent)
    {
        ...
        if ($relationsXml === false) {
            throw new \Maho\Search\Lucene\Exception('Invalid archive or corrupted .xlsx file.');
        }
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == \Maho\Search\Lucene\Document\OpenXml::SCHEMA_OFFICEDOCUMENT) {
                ...
                $workbookRelations = \Maho\Search\Lucene\Xml\Security::scan($package->getFromName( $this->absoluteZipPath(dirname($rel["Target"]) . "/_rels/" . basename($rel["Target"]) . ".rels")) );
                $workbookRelations->registerXPathNamespace("rel", \Maho\Search\Lucene\Document\OpenXml::SCHEMA_RELATIONSHIP);

                // Read shared strings
                $sharedStringsPath = $workbookRelations->xpath("rel:Relationship[@Type='" . \Maho\Search\Lucene\Document\Xlsx::SCHEMA_SHAREDSTRINGS . "']");
                $sharedStringsPath = (string)$sharedStringsPath[0]['Target'];
                $xmlStrings = \Maho\Search\Lucene\Xml\Security::scan($package->getFromName( $this->absoluteZipPath(dirname($rel["Target"]) . "/" . $sharedStringsPath)) );
                ...
                foreach ($workbookRelations->Relationship as $workbookRelation) {
                    if ($workbookRelation["Type"] == \Maho\Search\Lucene\Document\Xlsx::SCHEMA_WORKSHEETRELATION) {
                        $worksheets[ str_replace( 'rId', '', (string)$workbookRelation["Id"]) ] = \Maho\Search\Lucene\Xml\Security::scan(
     ...
     * @param string  $fileName
     * @param boolean $storeContent
     * @return \Maho\Search\Lucene\Document\Xlsx
     */
    public static function loadXlsxFile($fileName, $storeContent = false)
    {
        return new \Maho\Search\Lucene\Document\Xlsx($fileName, $storeContent);
    }
}
