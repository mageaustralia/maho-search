...
class Html extends \Maho\Search\Lucene\Document
{
    ...
    private function __construct($data, $isFile, $storeContent, $defaultEncoding = '')
    {
        ...
        $this->addField(\Maho\Search\Lucene\Field::Text('title', $docTitle, 'UTF-8'));

        $metaNodes = $xpath->query('/html/head/meta[@name]');
        foreach ($metaNodes as $metaNode) {
            $this->addField(\Maho\Search\Lucene\Field::Text($metaNode->getAttribute('name'),
        ...
        if ($storeContent) {
            $this->addField(\Maho\Search\Lucene\Field::Text('body', $docBody, 'UTF-8'));
        } else {
            $this->addField(\Maho\Search\Lucene\Field::UnStored('body', $docBody, 'UTF-8'));
        }
     ...
     * @param boolean $storeContent
     * @param string  $defaultEncoding   HTML encoding, is used if it's not specified using Content-type HTTP-EQUIV meta tag.
     * @return \Maho\Search\Lucene\Document\Html
     */
    public static function loadHTML($data, $storeContent = false, $defaultEncoding = '')
    {
        return new \Maho\Search\Lucene\Document\Html($data, false, $storeContent, $defaultEncoding);
    }
    ...
    {
        return new \Maho\Search\Lucene\Document\Html($file, true, $storeContent, $defaultEncoding);
    }
