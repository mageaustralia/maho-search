...
class SegmentInfo implements \Maho\Search\Lucene\Index\TermsStream\TermsStreamInterface
{
     ...
     *
     * Array of arrays (\Maho\Search\Lucene\Index\Term objects are represented as arrays because
     ...
     *
     * Array of arrays (\Maho\Search\Lucene\Index\TermInfo objects are represented as arrays because
     * of performance considerations)
     * [0] -> $docFreq
     * [1] -> $freqPointer
     * [2] -> $proxPointer
    ...
    /**
     * Segment fields. Array of \Maho\Search\Lucene\Index\FieldInfo objects for this segment
     *
     * @var array
     */
    private $_fields;
     ...
     * File system adapter.
     *
     * @var \Maho\Search\Lucene\Storage\Directory\Filesystem
     */
    private $_directory;
     ...
     * \Maho\Search\Lucene\Search\Similarity::encodeNorm())
     *
     * @var array
     */
    ...
    /**
     * \Maho\Search\Lucene\Index\SegmentInfo constructor
     *
     ...
     */
    public function __construct(\Maho\Search\Lucene\Storage\Directory $directory, $name, $docCount, $delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false, $isCompound = null)
    {
        ...
        for ($count=0; $count < $fieldsCount; $count++) {
            ...
            $this->_fields[$count] = new \Maho\Search\Lucene\Index\FieldInfo($fieldName,
            ...
            if ($fieldBits & 0x10) {
                ...
                $this->_norms[$count] = str_repeat(chr(\Maho\Search\Lucene\Search\Similarity::encodeNorm(1.0)), $docCount);
            }
     ...
     * @return \Maho\Search\Lucene\Index\FieldInfo
     */
    ...
    private function _loadDictionaryIndex()
    {
        ...
        list($this->_termDictionary, $this->_termDictionaryInfos) =
                    \Maho\Search\Lucene\Index\DictionaryLoader::load($tiiFileData);
     ...
     * @return \Maho\Search\Lucene\Index\TermInfo
     */
    public function getTermInfo(\Maho\Search\Lucene\Index\Term $term)
    {
        $termKey = $term->key();
        ...
        while ($highIndex >= $lowIndex) {
            ...
            if ($delta < 0) {
                ...
                $termInfo = new \Maho\Search\Lucene\Index\TermInfo($a[0], $a[1], $a[2], $a[3], $a[4]);
        ...
        for ($count = $prevPosition*$indexInterval + 1;
            ...
            $termValue        = \Maho\Search\Lucene\Index\Term::getPrefix($termValue, $termPrefixLength) . $termSuffix;
        ...
        if ($termFieldNum == $searchField && $termValue == $term->text) {
            $termInfo = new \Maho\Search\Lucene\Index\TermInfo($docFreq, $freqPointer, $proxPointer, $skipOffset);
        } else {
            $termInfo = null;
        }
     ...
     * @return array
     */
    public function termDocs(\Maho\Search\Lucene\Index\Term $term, $shift = 0, $docsFilter = null)
    {
        $termInfo = $this->getTermInfo($term);

        if (!$termInfo instanceof \Maho\Search\Lucene\Index\TermInfo) {
            if ($docsFilter !== null  &&  $docsFilter instanceof \Maho\Search\Lucene\Index\DocsFilter) {
...
// ---------------------------------------------------------------
     ...
     * @return \Maho\Search\Lucene\Index\TermInfo
     */
    public function termFreqs(\Maho\Search\Lucene\Index\Term $term, $shift = 0, $docsFilter = null)
    {
        $termInfo = $this->getTermInfo($term);
...
// ---------------------------------------------------------------
     ...
     * @return \Maho\Search\Lucene\Index\TermInfo
     */
    public function termPositions(\Maho\Search\Lucene\Index\Term $term, $shift = 0, $docsFilter = null)
    {
        $termInfo = $this->getTermInfo($term);
...
// ---------------------------------------------------------------
    ...
    public function norm($id, $fieldName)
    {
        ...
        return \Maho\Search\Lucene\Search\Similarity::decodeNorm( ord($this->_norms[$fieldNum][$id]) );
    }
     ...
     *
     * @var \Maho\Search\Lucene\Storage\File
     */
     ...
     *
     * @var \Maho\Search\Lucene\Index\TermInfo
     */
    ...
    public function resetTermsStream(/** $startId = 0, $mode = self::SM_TERMS_ONLY */)
    {
        ...
        $this->_lastTerm          = new \Maho\Search\Lucene\Index\Term('', -1);
        $this->_lastTermInfo      = new \Maho\Search\Lucene\Index\TermInfo(0, 0, 0, 0);
    ...
    public function skipTo(\Maho\Search\Lucene\Index\Term $prefix)
    {
        ...
        $this->_lastTerm     = new \Maho\Search\Lucene\Index\Term($prevTerm[1] /* text */,
                                                                 ($prevTerm[0] == -1) ? '' : $this->_fields[$prevTerm[0] /* field */]->name);
        $this->_lastTermInfo = new \Maho\Search\Lucene\Index\TermInfo($prevTermInfo[0] /* docFreq */,
    ...
    public function nextTerm()
    {
        ...
        $termValue        = \Maho\Search\Lucene\Index\Term::getPrefix($this->_lastTerm->text, $termPrefixLength) . $termSuffix;

        $this->_lastTerm = new \Maho\Search\Lucene\Index\Term($termValue, $this->_fields[$termFieldNum]->name);
        ...
        $this->_lastTermInfo = new \Maho\Search\Lucene\Index\TermInfo($docFreq, $freqPointer, $proxPointer, $skipOffset);
