...
class Writer
{
     ...
     * File system adapter.
     *
     * @var \Maho\Search\Lucene\Storage\Directory
     */
     ...
     * Array of \Maho\Search\Lucene\Index\SegmentInfo objects
     *
     * @var array
     */
     ...
     *
     * @var \Maho\Search\Lucene\Index\SegmentWriter\DocumentWriter
     */
    private $_currentSegment = null;

    /**
     * Array of \Maho\Search\Lucene\Index\SegmentInfo objects for this index.
     *
     * It's a reference to the corresponding \Maho\Search\Lucene\Lucene::$_segmentInfos array
     *
     ...
     */
    public static function createIndex(\Maho\Search\Lucene\Storage\Directory $directory, $generation, $nameCount)
    {
        if ($generation == 0) {
            ...
            $segmentsFile = $directory->createFile(\Maho\Search\Lucene\Lucene::getSegmentFileName($generation));
     ...
     * @param \Maho\Search\Lucene\Storage\File $cleanUpLock
     */
    public function __construct(\Maho\Search\Lucene\Storage\Directory $directory, &$segmentInfos, $targetFormatVersion)
    {
    ...
    public function addDocument(\Maho\Search\Lucene\Document $document)
    {
        ...
        if ($this->_currentSegment === null) {
            $this->_currentSegment =
                new \Maho\Search\Lucene\Index\SegmentWriter\DocumentWriter($this->_directory, $this->_newSegmentName());
        }
    ...
    {
        if (\Maho\Search\Lucene\LockManager::obtainOptimizationLock($this->_directory) === false) {
            return;
        }

        if (!$this->_hasAnythingToMerge()) {
            \Maho\Search\Lucene\LockManager::releaseOptimizationLock($this->_directory);
            return;
        }
    ...
    private function _updateSegments()
    {
        ...
        $generation = \Maho\Search\Lucene\Lucene::getActualGeneration($this->_directory);
        $segmentsFile   = $this->_directory->getFileObject(\Maho\Search\Lucene\Lucene::getSegmentFileName($generation), false);
        $newSegmentFile = $this->_directory->createFile(\Maho\Search\Lucene\Lucene::getSegmentFileName(++$generation), false);
        ...
        try {
            // Write format marker
            if ($this->_targetFormatVersion == \Maho\Search\Lucene\Lucene::FORMAT_2_1) {
                $newSegmentFile->writeInt((int)0xFFFFFFFD);
            } else if ($this->_targetFormatVersion == \Maho\Search\Lucene\Lucene::FORMAT_2_3) {
            ...
            if ($format == (int)0xFFFFFFFF) {
                $srcFormat = \Maho\Search\Lucene\Lucene::FORMAT_PRE_2_1;
            } else if ($format == (int)0xFFFFFFFD) {
                $srcFormat = \Maho\Search\Lucene\Lucene::FORMAT_2_1;
            } else if ($format == (int)0xFFFFFFFC) {
                $srcFormat = \Maho\Search\Lucene\Lucene::FORMAT_2_3;
            } else {
            ...
            for ($count = 0; $count < $segmentsCount; $count++) {
                ...
                if ($srcFormat == \Maho\Search\Lucene\Lucene::FORMAT_PRE_2_1) {
                    ...
                    if ($srcFormat == \Maho\Search\Lucene\Lucene::FORMAT_2_3) {
                ...
                if (!in_array($segName, $this->_segmentsToDelete)) {
                    ...
                    if ($this->_targetFormatVersion == \Maho\Search\Lucene\Lucene::FORMAT_2_3) {
                        ...
                        throw new \Maho\Search\Lucene\Exception('Index conversion to lower format version is not supported.');
                    }
        ...
        if (\Maho\Search\Lucene\LockManager::escalateReadLock($this->_directory)) {
            /**
             * Clean-up directory
             */
            $filesToDelete = array();
            ...
            foreach ($this->_directory->fileList() as $file) {
                if ($file == 'deletable') {
                    ...
                    if ($file != \Maho\Search\Lucene\Lucene::getSegmentFileName($generation)) {
                        $filesToDelete[] = $file;
            ...
            \Maho\Search\Lucene\LockManager::deEscalateReadLock($this->_directory);
        } else {
