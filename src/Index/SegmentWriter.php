...
abstract class SegmentWriter
{
    ...
    public function addField(\Maho\Search\Lucene\Field $field)
    {
        if (!isset($this->_fields[$field->name])) {
            ...
            $this->_fields[$field->name] =
                                new \Maho\Search\Lucene\Index\FieldInfo($field->name,
                                                                       $field->isIndexed,
                                                                       $fieldNumber,
     ...
     * Returns actual field number
     *
     * @param \Maho\Search\Lucene\Index\FieldInfo $fieldInfo
     * @return integer
     */
    public function addFieldInfo(\Maho\Search\Lucene\Index\FieldInfo $fieldInfo)
    {
        if (!isset($this->_fields[$fieldInfo->name])) {
            $fieldNumber = count($this->_fields);
            $this->_fields[$fieldInfo->name] =
                                new \Maho\Search\Lucene\Index\FieldInfo($fieldInfo->name,
                                                                       $fieldInfo->isIndexed,
                                                                       $fieldNumber,
     ...
     *
     * @param array $storedFields array of \Maho\Search\Lucene\Field objects
     */
...
// not supported yet            0x08 /* term offsets are stored with the term vectors */   |
     ...
     *
     * @param \Maho\Search\Lucene\Index\Term $termEntry
    ...
    public function addTerm($termEntry, $termDocs)
    {
        ...
        $term = new \Maho\Search\Lucene\Index\Term($termEntry->text,
                                                  $this->_fields[$termEntry->field]->number);
        $termInfo = new \Maho\Search\Lucene\Index\TermInfo(count($termDocs),
     ...
     *
     * @param \Maho\Search\Lucene\Storage\File $dicFile
     * @param \Maho\Search\Lucene\Index\Term $prevTerm
     * @param \Maho\Search\Lucene\Index\Term $term
     * @param \Maho\Search\Lucene\Index\TermInfo $prevTermInfo
     * @param \Maho\Search\Lucene\Index\TermInfo $termInfo
     */
    protected function _dumpTermDictEntry(\Maho\Search\Lucene\Storage\File $dicFile,
                                        &$prevTerm,     \Maho\Search\Lucene\Index\Term     $term,
                                        &$prevTermInfo, \Maho\Search\Lucene\Index\TermInfo $termInfo)
    {
