...
class Lucene implements \Maho\Search\Lucene\LuceneInterface
{
     ...
     *
     * @var \Maho\Search\Lucene\Index\Writer
     */
    private $_writer = null;

    /**
     * Array of \Maho\Search\Lucene\Index\SegmentInfo objects for current version of index.
     *
    ...
    {
        /**
         * \Maho\Search\Lucene\Lucene uses segments.gen file to retrieve current generation number
         *
        ...
        try {
            for ($count = 0; $count < self::GENERATION_RETRIEVE_COUNT; $count++) {
                ...
                if ($format != (int)0xFFFFFFFE) {
                    throw new \Maho\Search\Lucene\Exception('Wrong segments.gen file format');
                }
            ...
            throw new \Maho\Search\Lucene\Exception('Index is under processing now');
     ...
     *
     * @param \Maho\Search\Lucene\Storage\Directory\Filesystem|string $directory
     * @throws \Maho\Search\Lucene\Exception
     * @throws \Maho\Search\Lucene\Exception
     */
    public function __construct($directory = null, $create = false)
    {
        ...
        \Maho\Search\Lucene\LockManager::obtainReadLock($this->_directory);
        ...
        if ($create) {
            try {
                ...
                \Maho\Search\Lucene\LockManager::releaseReadLock($this->_directory);

                if (strpos($e->getMessage(), 'Can\'t obtain exclusive index lock') === false) {
                    throw new \Maho\Search\Lucene\Exception($e->getMessage(), $e->getCode(), $e);
                } else {
                    throw new \Maho\Search\Lucene\Exception('Can\'t create index. It\'s under processing now', 0, $e);
                }
            }
    ...
    /**
     * Returns an instance of \Maho\Search\Lucene\Index\Writer for the index
     *
     * @return \Maho\Search\Lucene\Index\Writer
     */
     ...
     *
     * @param \Maho\Search\Lucene\Search\QueryParser|string $query
    ...
    public function find($query)
    {
        ...
        foreach ($query->matchedDocs() as $id => $num) {
            ...
            if( $docScore != 0 ) {
                $hit = new \Maho\Search\Lucene\Search\QueryHit($this);
                $hit->id = $id;
                $hit->score = $docScore;
        ...
        if (func_num_args() == 1) {
            ...
            for ($count = 1; $count < count($argList); $count++) {
                ...
                if (!is_string($fieldName)) {
                    throw new \Maho\Search\Lucene\Exception('Field name must be a string.');
                }
                ...
                if (strtolower($fieldName) == 'score') {
                    ...
                    if (!in_array($fieldName, $fieldNames)) {
                        throw new \Maho\Search\Lucene\Exception('Wrong field name.');
                    }
    ...
    {
        if ($id instanceof \Maho\Search\Lucene\Search\QueryHit) {
            /* @var $id \Maho\Search\Lucene\Search\QueryHit */
            $id = $id->id;
        }
        ...
        $doc = new \Maho\Search\Lucene\Document();
        for ($count = 0; $count < $fieldCount; $count++) {
            ...
            if (!($bits & 2)) { // Text data
                $field = new \Maho\Search\Lucene\Field($fieldInfo->name,
                                                      $fdtFile->readString(),
                                                      'UTF-8',
                                                      true,
                                                      $fieldInfo->isIndexed,
    ...
    {
        $segmentStartDocId = 0;
        $result = new \Maho\Search\Lucene\Index\DocsFilter();
     ...
     *
     * @var \Maho\Search\Lucene\TermStreamsPriorityQueue
     */
