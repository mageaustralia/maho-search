...
class BooleanQuery extends \Maho\Search\Lucene\Search\Query
{
    ...
    /**
     * Subqueries
     * Array of \Maho\Search\Lucene\Search\Query
     *
     * @var array
     */
    ...
    /**
     * Add a $subquery (\Maho\Search\Lucene\Search\Query) to this query.
     *
     ...
     *
     * @param  \Maho\Search\Lucene\Search\Query $subquery
     * @param  boolean|null $sign
     * @return void
     */
    public function addSubquery(\Maho\Search\Lucene\Search\Query $subquery, $sign=null) {
    ...
    public function optimize(\Maho\Search\Lucene\LuceneInterface $index)
    {
        ...
        foreach ($subqueries as $id => $subquery) {
            if ($subquery instanceof \Maho\Search\Lucene\Search\Query\Insignificant) {
        ...
        $optimizedQuery = new \Maho\Search\Lucene\Search\Query\BooleanQuery($subqueries, $signs);
        ...
        foreach ($subqueries as $id => $subquery) {
            if ($subquery instanceof \Maho\Search\Lucene\Search\Query\Term) {
                ...
                unset($subqueries[$id]);
                unset($signs[$id]);
           } else if ($subquery instanceof \Maho\Search\Lucene\Search\Query\MultiTerm) {
     ...
     * @return float
     */
    public function _nonConjunctionScore($docId, \Maho\Search\Lucene\LuceneInterface $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = array();
